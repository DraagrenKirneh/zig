...
pub fn fetchAndAddDependencies(
    ...
    for (manifest.dependencies.keys(), 0..) |name, i| {
        ...
        if (!pkg.table.contains(name)) {
            try pkg.add(gpa, name, sub_pkg);
        }
        if (!root_pkg.table.contains(fqn)) {
...
fn fetchAndUnpack(
    ...
    const actual_hash = a: {
        ...
        // const ct = req.response.headers.get("Content-Type");
        // _ = ct;
        ...
        var x = (try req.response.headers.getValues(gpa, "Content-Type")).?;
        defer gpa.free(x);
